...
public class LiveMappingDescription<T> {
  ...
  public <G extends ExpressionGenerator<T>> MappedDeleteExpression<T, G> generateDeleteExpression(
      T object, G generator, String table) {
    Map<String, AttributeValue> keys = this.exportKeys(object);
    return new MappedDeleteExpression<>(generator, table, keys, null, this);
  }

  public <G extends ExpressionGenerator<T>> MappedScanExpression<T, G> generateScanExpression(
      G generator, String table) {
    return new MappedScanExpression<>(generator, table, null, this);
  }

  public <G extends ExpressionGenerator<T>> MappedUpdateExpression<T, G> generateUpdateExpression(
      T object, G generator, String table) {
    MappedUpdateExpression.RczSetExpressionGenerator<T> ddd =
        new MappedUpdateExpression.RczSetExpressionGenerator<>();

    List<MappedUpdateExpression.Statement<T>> setExpressions =
        fields.stream()
            .filter(it -> !it.isKeyValue())
            .filter(it -> it.getExport().apply(object).isPresent())
            .map(
                it ->
                    MappedUpdateExpression.UpdateStatement.<T>builder()
                        // Argument G - is not important
                        .path(
                            new MappedUpdateExpression.RczPathExpression<>(
                                PrimitiveElement.<T, G>builder()
                                    .selectedElement(it.getDdbName())
                                    .build()))
                        .override(true)
                        .value(
                            ddd.just(
                                it.getExport().apply(object).orElseThrow(RuntimeException::new)))
                        .build())
            .collect(Collectors.toList());
  ...
  public T transform(Map<String, AttributeValue> m) {
    ...
    for (Map.Entry<String, AttributeValue> e : m.entrySet()) {
      ...
      initialObject = dict.get(key).getWither().apply(initialObject, value);
    }
  ...
  public Map<String, AttributeValue> export(T object) {
    return fields.stream()
        .collect(
            Collectors.toMap(
                FieldMappingDescription::getDdbName, it -> it.getExport().apply(object)))
        .entrySet()
        .stream()
