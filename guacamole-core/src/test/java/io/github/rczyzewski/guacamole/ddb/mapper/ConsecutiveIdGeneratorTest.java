...
class ConsecutiveIdGeneratorTest {
  ...
  void sunnyDay() {
    ...
    List<String> values = Stream.generate(idGenerator).limit(12).collect(Collectors.toList());

    Assertions.assertThat(values)
        .isEqualTo(
            Arrays.asList("A", "B", "C", "D", "AA", "AB", "AC", "AD", "BA", "BB", "BC", "BD"));
  }

  @Test
  void smallBase() {
    ConsecutiveIdGenerator idGenerator = ConsecutiveIdGenerator.builder().base("A").build();
    List<String> values = Stream.generate(idGenerator).limit(1000).collect(Collectors.toList());
    ...
    IntStream.range(0, 1000)
        .forEach(
            i -> Assertions.assertThat(values.get(i)).isEqualTo(generateStringOfA.apply(i + 1)));
  }

  @Test
  void emptyBase() {
    ConsecutiveIdGenerator idGenerator = ConsecutiveIdGenerator.builder().base("").build();
    Assertions.assertThatThrownBy(idGenerator::get)
        .isInstanceOf(ConsecutiveIdGenerator.UnsupportedArgumentException.class);
  }
}
